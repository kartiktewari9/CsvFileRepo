...
using System.IO;
using System.Text;
using CsvFileProject.Common.Services;
...
namespace CsvTest
{
    ...
    public class TestHarness
    {
        ...
        private void TestReader()
        {
            ...
            TestReaderLine("", false, EmptyLineBehaviorEnum.EmptyColumn);
            TestReaderLine("", false, EmptyLineBehaviorEnum.EndOfFile);
            TestReaderLine("", false, EmptyLineBehaviorEnum.Ignore);
            TestReaderLine("", false, EmptyLineBehaviorEnum.NoColumns);

            columns = TestReaderLine("\r\n123,456\r\n", true, EmptyLineBehaviorEnum.EmptyColumn);
            Debug.Assert(columns.Count == 1);
            Debug.Assert(columns[0] == "");

            TestReaderLine("\r\n123,456\r\n", false, EmptyLineBehaviorEnum.EndOfFile);

            columns = TestReaderLine("\r\n\r\n\r\n\r\n\r\n123,456\r\n", true, EmptyLineBehaviorEnum.Ignore);
            Debug.Assert(columns.Count == 2);
            Debug.Assert(columns[0] == "123");
            Debug.Assert(columns[1] == "456");

            columns = TestReaderLine("\r\n123,456\r\n", true, EmptyLineBehaviorEnum.NoColumns);
        ...
        private List<string> TestReaderLine(string line, bool expectedReturnValue = true, EmptyLineBehaviorEnum EmptyLineBehaviorEnum = EmptyLineBehaviorEnum.NoColumns)
        {
            List<string> columns = new List<string>();
            bool result;
            using (var readerService = new CsvFileReaderService())
            {
                readerService.Reader = new StreamReader(new MemoryStream(Encoding.UTF8.GetBytes(line)));
                readerService.EmptyLineBehaviorEnum = EmptyLineBehaviorEnum;
                result = readerService.ReadRow(columns);
        ...
        private string TestWriterLine(params string[] columns)
        {
            ...
            using(CsvFileWriterService writerService=new CsvFileWriterService())
            {
                writerService.Writer = new StreamWriter(stream);
                writerService.WriteRow(new List<string>(columns));
            }
            // Get data written
